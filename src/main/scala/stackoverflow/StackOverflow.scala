...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(Int, Iterable[(Posting, Posting)])] = {
    ...
    val answers = postings.filter(p => p.postingType == 1).map(p => (p.id, p))
    val questions = postings.filter(p => p.postingType == 2 && p.parentId.isDefined).map(p => (p.parentId.get, p))
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val closest = vectors.map(p =>
      (findClosest(p, means), p)).groupByKey()
  ...
  def computeMetrics(vs: Iterable[(Int, Int)]): (String, Double, Int, Int) = {
